...
class SSAD_SCDM(object):
    ...
    def generate_all_anchor(self):
        ...
        return all_anchor_list#(5,256/128/64/32/16,4,2)

# according to anchor length, reverse anchor from range 1 to 64
    def generate_anchor_mask(self):
        all_anchor = self.generate_all_anchor()#(5,256/128/64/32/16,4,2)
        length = len(all_anchor)#5
        output_list = []
        for i in range(length):
            anchor_group_shape = np.shape(all_anchor[i])#(256/128/64/32/16,4,2)
            ...
            for ii in range(anchor_group_shape[0]):
                for jj in range(anchor_group_shape[1]):
                    ...
                    if all_anchor[i][ii][jj][1] - all_anchor[i][ii][jj][0] > 64.0:
                        anchor_mask[ii,jj] = 0.0
            anchor_mask = tf.convert_to_tensor(anchor_mask)#choose anchor length of(256/128/64/32/16,4)
            anchor_mask = tf.cast(anchor_mask,tf.float32)
            anchor_mask = tf.expand_dims(tf.expand_dims(anchor_mask,0),1)#(1,256/128/64/32/16,4)
            anchor_mask = tf.tile(anchor_mask,[self.options['batch_size'],1,1,1])#(batchsize,256/128/64/32/16,4)
            output_list.append(anchor_mask)#(batchsize,256/128/64/32/16,4)
        return output_list



    def predict(self,anchor1,is_training,anchor_id):#anchor1:(batch_size,256/128/32/16,512)
        ...
        #(batch_size, 256/128/64/32 ,4*2)
        predict1_reg = tf.nn.tanh(predict1_reg)# Numerical compression to 0-1
        return predict1_overlap,predict1_reg

    def predict_label(self, anchor, is_training, anchor_id):
        predict1_label = bn_relu_conv(anchor,is_training,512,64,1,3,1,name = 'predict%d_1_label'%(anchor_id))
        predict1_label = bn_relu_conv(predict1_label,is_training,64,len(self.options['scale_ratios_anchor%d'%(anchor_id)])*len(self.options['facial_dict']),1,3,1,name='predict%d_2_label'%(anchor_id))
        return predict1_label
    ...
    def network(self, feature_segment, sentence_fts, word_sequence, word_len, batch_size, is_training=False, reuse=False):
        ...
        with tf.variable_scope('label_prediction_layer',reuse = reuse) as scope:
            predict1_label = self.predict_label(anchor1,is_training,1)
            predict2_label = self.predict_label(anchor2,is_training,2)
            predict3_label = self.predict_label(anchor3,is_training,3)
            predict4_label = self.predict_label(anchor4,is_training,4)
            predict5_label = self.predict_label(anchor5,is_training,5)#(batch_size,16,4*24)

   
        predict_overlap = [predict1_overlap,predict2_overlap,predict3_overlap,predict4_overlap,predict5_overlap]#(5,bs,)
        predict_reg = [predict1_reg,predict2_reg,predict3_reg,predict4_reg,predict5_reg]
        predict_label_list = [predict1_label, predict2_label, predict3_label, predict4_label, predict5_label]
        return predict_overlap,predict_reg, predict_label_list



    def build_dev_inference(self,is_training=False,reuse=False):
        inputs={}
        ...
        gt_label_map = tf.placeholder(tf.float32,[self.options['batch_size'],len(self.options['feature_map_len']),max(self.options['feature_map_len']),len(self.options['scale_ratios_anchor1'])*len(options['facial_dict'])])
        inputs['gt_facial_map'] = gt_label_map
        ...
        predict_overlap,predict_reg, predict_label_map = self.network(feature_segment, sentence_fts, word_sequence, sentence_w_len, batch_size = self.options['batch_size'],is_training=is_training,reuse=reuse)
        facial_map_true = {}
        facial_map_true_true = {}
        for key_facial in self.options['facial_dict']:
            facial_map_true[key_facial] = 0.0
            facial_map_true_true[key_facial] = 0.0
            
        predict_overlap = [tf.nn.sigmoid(i) for i in predict_overlap]
        predict_label_map = [tf.nn.sigmoid(i) for i in predict_label_map]
        for i in range(len(self.options['feature_map_len'])):
            one = tf.ones_like(predict_label_map[i],dtype = tf.bool)
            zero = tf.zeros_like(predict_label_map[i], dtype = tf.bool)
            predict_label_map[i] = tf.where(predict_label_map[i] <0.5, x=zero, y=one)
 
            for key_facial in self.options['facial_dict'].keys():
                single_predict_facial_label = predict_label_map[i][:, :, :self.options['feature_map_len'][i], int(self.options['facial_dict'][key_facial]):len(self.options['scale_ratios_anchor%d'%(i+1)])*len(self.options['facial_dict']):len(self.options['facial_dict'])]#(bs,1,256/128,4)

                single_gt_facial_label_tmp = tf.cast(gt_label_map[:, i:i+1, :self.options['feature_map_len'][i], int(self.options['facial_dict'][key_facial]):len(self.options['scale_ratios_anchor%d'%(i+1)])*len(self.options['facial_dict']):len(self.options['facial_dict'])], tf.bool)#(bs,1,256/128,4)
                
                correct_pred = tf.equal(single_predict_facial_label, single_gt_facial_label_tmp)
                accurate_num = tf.reduce_sum(tf.cast(correct_pred, tf.float32))
                correct_pred_true_true = tf.logical_and(single_predict_facial_label, single_gt_facial_label_tmp)
                precition_num = tf.reduce_sum(tf.cast(correct_pred_true_true, tf.float32))
                facial_map_true[key_facial] += accurate_num 
                facial_map_true_true[key_facial] += precition_num   
        
        
        return inputs,predict_overlap,predict_reg,facial_map_true, facial_map_true_true

    def build_test_inference(self,is_training=False,reuse=False):
        inputs={}
        ...
        predict_overlap,predict_reg, predict_label_map_tmp = self.network(feature_segment, sentence_fts, word_sequence, sentence_w_len, batch_size = self.options['batch_size'],is_training=is_training,reuse=reuse)
    ...
    def build_train(self,is_training=True):
        ...
        predict_overlap,predict_reg, predict_label_map = self.network(feature_segment,sentence_fts,word_sequence,sentence_w_len,batch_size = self.options['batch_size'],is_training=is_training)
        #predict_label_map:(4,batchsize,256/128/64/32/16,24*4)
        #predict_overlap:(4,batchsize,256/128/64/32/16,3*4)
        ...
        positive_facial_loss_list = []
        negative_facial_loss_list = []
        
        for i in range(len(self.options['feature_map_len'])):#5
            facial_entropy_positive_loss_dict = {}
            facial_entropy_negative_loss_dict = {}
#positive loss
            ...
            single_gt_overlap_temp = tf.identity(single_gt_overlap)#(bs,1,256/128,4)
            
            #(b,1,map_len,4)
            
            ones_now = np.ones([self.options['batch_size'],1,self.options['feature_map_len'][i],len(self.options['scale_ratios_anchor%d'%(i+1)])],np.float32)#(b,1,256/128,4)
            zeros_now = np.zeros([self.options['batch_size'],1,self.options['feature_map_len'][i],len(self.options['scale_ratios_anchor%d'%(i+1)])],np.float32)#(b,1,256/128,4)
...
#negative loss
            ...
            for key_facial in self.options['facial_dict'].keys():
                if not key_facial in facial_entropy_positive_loss_dict.keys():
                    facial_entropy_positive_loss_dict[key_facial] = 0.0
                if not key_facial in facial_entropy_negative_loss_dict.keys():
                    facial_entropy_negative_loss_dict[key_facial] = 0.0

                single_predict_facial_label = predict_label_map[i][:, :, :self.options['feature_map_len'][i], int(self.options['facial_dict'][key_facial]):len(self.options['scale_ratios_anchor%d'%(i+1)])*len(self.options['facial_dict']):len(self.options['facial_dict'])]#(bs,1,256/128,4)

                single_gt_facial_label = gt_label_map[:, i:i+1, :self.options['feature_map_len'][i], int(self.options['facial_dict'][key_facial]):len(self.options['scale_ratios_anchor%d'%(i+1)])*len(self.options['facial_dict']):len(self.options['facial_dict'])]#(bs,1,256/128,4)
                single_gt_facial_label_tmp = tf.identity(single_gt_facial_label)
                single_gt_facial_label_positive = tf.where(single_gt_facial_label_tmp > 0.5,ones_now,zeros_now)
                single_gt_facial_label_positive = single_gt_facial_label_positive * self.anchor_mask_list[i]
                facial_positive_num = tf.reduce_sum(single_gt_facial_label_positive)
                
                facial_positive_loss = tf.reduce_sum(tf.multiply(tf.nn.sigmoid_cross_entropy_with_logits(logits=single_predict_facial_label,labels=single_gt_facial_label_tmp),single_gt_facial_label_positive))
                facial_positive_loss = tf.cond(tf.greater(facial_positive_num,tf.constant(0.0)), lambda: facial_positive_loss/facial_positive_num, lambda: facial_positive_loss)
                facial_entropy_positive_loss_dict[key_facial] += facial_positive_loss
                
                single_gt_facial_label_negative = tf.where(single_gt_facial_label_tmp <0.5,ones_now,zeros_now)
                single_gt_facial_label_negative = single_gt_facial_label_negative * self.anchor_mask_list[i]
                facial_negative_num = tf.reduce_sum(single_gt_facial_label_negative)

                facial_negative_loss =  tf.reduce_sum(tf.multiply(tf.nn.sigmoid_cross_entropy_with_logits(logits=single_predict_facial_label,labels=single_gt_facial_label_tmp),single_gt_facial_label_negative))
                facial_negative_loss = tf.cond(tf.greater(facial_negative_num,tf.constant(0.0)), lambda: facial_negative_loss/facial_negative_num, lambda: facial_negative_loss)
                facial_entropy_negative_loss_dict[key_facial] += facial_negative_loss
            ...
            anchor_predict_loss.append((self.options['posloss_weight']*positive_loss+\
                                        ...
                                        self.options['facialposloss_weigth']*sum(facial_entropy_positive_loss_dict.values())+\
                                        self.options['facialnegloss_weigth']*sum(facial_entropy_negative_loss_dict.values()))
            ...
            positive_facial_loss_list.append(self.options['facialposloss_weigth']*sum(facial_entropy_positive_loss_dict.values()))
            negative_facial_loss_list.append(self.options['facialnegloss_weigth']*sum(facial_entropy_negative_loss_dict.values()))
        ...
        positive_facial_loss_all = sum(positive_facial_loss_list)
        negative_facial_loss_all = sum(negative_facial_loss_list)
        ...
        outputs['predict_facial_map'] = predict_label_map
        outputs['facial_positive_loss_all'] = positive_facial_loss_all
        outputs['facial_negative_loss_all'] = negative_facial_loss_all
        outputs['facial_possitive_num'] = facial_positive_num
        outputs['facial_possitive_loss'] = facial_positive_loss
        outputs['facial_negative_num'] = facial_negative_num
        outputs['facial_negative_loss'] = facial_negative_loss
        outputs['facial_map_true'] = facial_map_true
        outputs['facial_map_true_true'] = facial_map_true_true
