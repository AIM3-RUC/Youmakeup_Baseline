...
class ImgTextCompositionBase(torch.nn.Module):
  ...
  def compute_soft_triplet_loss_(self, mod_img1, img2):  # see as formulation (5) in paper
    triplets = []
...
'''
extract image features and text features
'''
class ImgEncoderTextEncoderBase(ImgTextCompositionBase):
  ...
  def __init__(self, texts, embed_dim):
    ...
    img_model.fc = torch.nn.Sequential(torch.nn.Linear(512, embed_dim))# embed_dim = 512
    self.img_model = img_model

    # text model
    self.text_model = text_model.TextLSTMModel(
        texts_to_build_vocab=texts,
        word_embed_dim=embed_dim, # embed_dim = 512
...
class TIRGLastConv(ImgEncoderTextEncoderBase):
  ...
  def compose_img_text(self, imgs, texts):
    ...
    x = self.a[0] * F.sigmoid(tgate) * x + self.a[1] * t # formulation (1) in paper
